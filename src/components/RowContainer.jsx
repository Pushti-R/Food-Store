  ...
  return (
    ...
    className={`w-full flex items-center gap-3 my-12 scroll-smooth ${flag ? 'overflow-x-scroll scrollbar-none' : 'overflow-x-hidden flex-wrap justify-center'}`}>
        {data && data.length > 0 ? (data.map((item) => (
            <div key={item?.id} className='w-275 h-[215px] min-w-[275px] md:w-300 md:min-w-[300px] bg-cardOverlay rounded-lg p-2 my-12 flex flex-col items-center justify-between backdrop-blur-lg hover:drop-shadow-lg'>
            ...
            <div className='w-full flex flex-col items-end justify-end'>
                ...
                <div className='flex items-center gap-8'>
                    <p className='text-lg text-headingColor font-semibold'>
                        <span className='text-sm text-red-500 px-2'>$</span>{item?.price}
                    </p>
                </div>
            </div>
        </div>
                ...
                <p className='text-xl text-headingColor font-semibold my-1'>Items Not Found</p>
            </div>
            )}
    </div>
  )
