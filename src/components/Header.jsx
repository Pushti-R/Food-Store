    ...
    <header className='fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16 bg-primary'>
        <div className='hidden md:flex w-full h-full items-center justify-between'>
          ...
          <div className='flex items-center gap-8'>
            ...
            <motion.img whileTap={{scale: 0.6}} src= {user ? user.photoURL : Avatar} alt="profile" onClick={login} className=' w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer rounded-full' />
            {
              isMenu && (
              ...
              {
                user && user.email === "pushti.ratanghayra@gmail.com" && (
                  <Link to={"/createItem"}>
          ...
          <div className='relative'>
            <motion.img whileTap={{scale: 0.6}} src= {user ? user.photoURL : Avatar} alt="profile" onClick={login} className=' w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer rounded-full' />
            {
                ...
                <motion.div initial = {{opacity : 0, scale : 0.6}} animate = {{opacity : 1, scale : 1}} exit = {{opacity : 0, scale : 0.6}} className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0'> 
              {
                user && user.email === "pushti.ratanghayra@gmail.com" && (
                  <Link to={"/createItem"}>
                    <p className='px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base'onClick={logout}>New Item <MdAdd /> </p>
                  </Link> 
                )
              }
